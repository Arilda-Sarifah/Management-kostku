...
public class LaporanView extends JPanel {
    ...
    private void initComponents() {
        ...
        btnExportCsv = UIUtilities.createModernButton("Export CSV", UIUtilities.SECONDARY_COLOR, Color.WHITE);
        btnExportCsv.setPreferredSize(new Dimension(120, 34));
        btnRefresh = UIUtilities.createModernButton("Refresh Laporan", UIUtilities.PRIMARY_COLOR, Color.WHITE);
        btnRefresh.setPreferredSize(new Dimension(140, 34));
        ...
        btnExportCsv.addActionListener(e -> exportCsvReport());
    }

    /**
     * Export data laporan ke file CSV (di project root).
     */
    public void exportCsvReport() {
        try {
            ...
            java.io.File out = new java.io.File("laporan_penghuni.csv");
            try (java.io.FileWriter fw = new java.io.FileWriter(out)) {
                // header
                for (int c = 0; c < model.getColumnCount(); c++) {
                    fw.append(model.getColumnName(c));
            ...
            JOptionPane.showMessageDialog(this, "Export CSV selesai: " + out.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
